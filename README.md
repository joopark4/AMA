...
> ⚠️ **주의 — 비공개 자산 안내**
>
> 본 저장소에는 라이선스상 재배포가 제한되거나 작가/플랫폼의 사용 약관에 따라 공개할 수 없는 자산이 **포함되어 있지 않습니다.** 대표적으로:
>
> - **Mixamo FBX 모션 파일** (`motions/mixamo/*.fbx`) — Adobe Mixamo 약관상 개인 사용 한정, 재배포 금지
> - **기본 VRM 아바타** (`src-tauri/assets/default.vrm`) — 작자 저작권
>
> 소스 빌드를 직접 진행하시려면 위 자산들을 **각자 환경에서 준비·재구성한 뒤** 빌드해야 합니다. 자산 없이 빌드하면 `default.vrm` 임베딩이 비어 기본 아바타가 표시되지 않거나, 모션 카탈로그가 비어 일부 동작이 동작하지 않을 수 있습니다. 자세한 자산 준비는 [에셋 라이선스 가이드](#vrm-파일-구하기구매-가이드)와 각 카테고리 안내를 참고하세요.
...
설정 패널은 우측 슬라이드 카드 형태로 열리며, 패널 **상단**에는 계정 정보(OAuth 로그인/약관 동의/계정 삭제) 카드가 항상 노출됩니다. 그 아래 본문은 다음 14개 섹션으로 구성되며, 각 카드는 펼침/접힘이 가능하고 펼침 상태는 다음 실행에 재현됩니다.

| # | 섹션 | 주요 기능 |
|---|---|---|
| 1 | **언어** | 앱 UI 언어 선택 (한국어 / English / 日本語) |
| 2 | **프리미엄 음성** | Supertone API 클라우드 TTS (베타 — 로그인 사용자 공유 풀) · 음성/모델/스타일/사용량 카드 |
| 3 | **AI 모델** | LLM provider 선택 — Ollama · LocalAI(로컬) / Claude · OpenAI · Gemini(클라우드) / OpenAI Codex CLI · Gemini CLI(로컬 CLI 에이전트). **Codex / Gemini CLI 선택 시 해당 CLI 전용 옵션(연결 상태 / 모델 / 추론 성능 / 작업 폴더 / 접근 권한)이 본 섹션 안에 펼쳐집니다.** |
| 4 | **Claude Code Channels** | MCP 서버 자동 등록 + 외부 Claude Code 세션과 아바타의 양방향 대화 |
| 5 | **오디오 & 마이크** | 마이크 입력 / 스피커 출력 디바이스 독립 선택 + 마이크 피크 미터 |
| 6 | **아바타** | VRM 파일 교체 / 표정 / 초기 시선 / 자유 이동 모드 / 말풍선 위치 / 애니메이션 / 물리 / 조명 |
| 7 | **캐릭터** | 캐릭터 프리셋(아키타입) / 성격 키워드 / 감정 표현 성향 / 존댓말 / 예시 대화 / 자발적 대화 토글 |
| 8 | **음성 (로컬)** | STT 엔진(Whisper) · 모델 선택(base/small/medium) + 로컬 TTS(Supertonic) 음성·출력 언어 + 글로벌 음성 단축키 |
| 9 | **화면 관찰** | 주기적 화면 캡처 + Vision LLM 분석으로 아바타가 능동 발화 — 캡처 대상 / 관찰 간격 / 응답 스타일 / 조용한 시간 / 권한 요청 |
| 10 | **모니터** | 멀티 모니터 환경에서 아바타를 표시할 모니터 선택 |
| 11 | **자주 쓰는 기능** | 자주 쓰는 설정 토글을 등록 → 우하단 ✨ 아이콘 또는 음성으로 즉시 호출 |
| 12 | **앱 업데이트** | 현재 버전 표시 / 업데이트 확인 / 다운로드 진행률 / 재시작 (24시간 자동 재확인) |
| 13 | **데이터 관리** | 다운로드된 AI 모델 데이터 관리(앱 삭제 시 자동 제거되지 않음) |
| 14 | **오픈소스 / 모델 라이선스** | 사용 라이브러리·AI 서비스·모델의 라이선스와 공식 링크 |

> 설정 패널은 우하단 톱니 아이콘 또는 macOS 메뉴바 `AMA → Settings...` (`⌘,`)로 열 수 있습니다.
...
AMA 아바타를 Google Gemini CLI(ACP)와 연결하여 코딩 에이전트와 양방향 대화가 가능합니다.

#### 사전 요구사항

- [Gemini CLI](https://github.com/google-gemini/gemini-cli) 설치 (`npm install -g @google/gemini-cli`)
- Gemini 인증 완료 (`gemini auth login` — Google OAuth 또는 API 키)
- Node.js 20+

#### 설치 상태 확인

```bash
gemini --version       # 버전 출력
gemini auth print      # 인증 상태 확인
```

#### 설정 단계

1. AMA 앱 실행
2. `설정 > AI 모델 > Gemini CLI` 선택
3. CLI 설치 상태와 인증 상태가 자동으로 확인됩니다
4. 연결되면 추가 설정 가능:
   - **모델**: 연결 후 `session/new` 응답으로 받은 사용 가능한 모델 목록에서 선택
   - **작업 폴더**: Gemini CLI가 파일을 읽고 쓸 디렉터리 (미지정 시 `~/Documents`)
   - **승인 모드** — 도구 실행 시 권한 처리 정책:
     - `default` — 위험 작업마다 자동 거부 (안전)
     - `auto_edit` — 파일 편집은 자동 승인, 명령 실행은 거부
     - `yolo` — 모든 도구 실행을 자동 승인 (신뢰 환경 한정)
     - `plan` — 실행 없이 계획만 표시
5. AMA에서 대화 → Gemini CLI가 코드 작업 수행 후 응답 → 아바타 TTS

#### 주의사항

- Gemini CLI는 백그라운드에서 `gemini --experimental-acp`를 자동으로 실행합니다. 별도 터미널 작업이 필요 없습니다.
- `yolo` 모드는 파일 편집·명령 실행·터미널 작업 모두 자동 승인합니다. **신뢰할 수 있는 환경에서만 사용**하세요.
- 작업 폴더 외부 파일 접근은 차단됩니다 (보안 격리).
- 화면 관찰(Screen Watch)이 활성화된 경우 캡처 이미지가 ACP `image` ContentBlock으로 전달됩니다.
- Provider를 다른 모델로 전환하면 Gemini CLI 연결이 자동으로 종료됩니다.
