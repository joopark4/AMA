...
> ⚠️ **注意 — 非公開アセットのご案内**
>
> 本リポジトリには、ライセンス上再配布が制限される、または作者・プラットフォームの利用規約に従って公開できないアセットが**含まれていません**。代表例:
>
> - **Mixamo FBX モーションファイル**（`motions/mixamo/*.fbx`） — Adobe Mixamo 規約により個人利用限定、再配布禁止
> - **デフォルト VRM アバター**（`src-tauri/assets/default.vrm`） — 作者著作権
>
> ソースビルドを直接行う場合は、上記アセットを**各自の環境で準備・再構成した上で**ビルドしてください。アセットなしでビルドすると、`default.vrm` の埋め込みが空となり既定アバターが表示されない、またはモーションカタログが空となり一部の動作が機能しない可能性があります。詳細は [VRM 入手ガイド](#vrm-ファイルの入手購入ガイド) と各カテゴリ案内をご参照ください。
...
設定パネルは右側からスライドして開きます。パネル**上部**には常にアカウントカード（OAuth ログイン / 利用規約 / アカウント削除）が表示されます。その下の本文は以下の 14 セクションで構成され、各カードは開閉可能で、開閉状態は次回起動時に再現されます。

| # | セクション | 主な機能 |
|---|---|---|
| 1 | **言語** | アプリ UI 言語（한국어 / English / 日本語） |
| 2 | **プレミアム音声** | Supertone API クラウド TTS（ベータ — ログインユーザー共有プール）。音声 / モデル / スタイル / 使用量カード |
| 3 | **AI モデル** | LLM プロバイダ選択 — Ollama · LocalAI（ローカル）/ Claude · OpenAI · Gemini（クラウド）/ OpenAI Codex CLI · Gemini CLI（ローカル CLI エージェント）。**Codex / Gemini CLI を選択すると、その CLI 専用オプション（接続状態 / モデル / 推論性能 / 作業フォルダ / アクセス権限）が本セクション内で展開されます。** |
| 4 | **Claude Code Channels** | MCP サーバー自動登録 + 外部 Claude Code セッションとアバターの双方向対話 |
| 5 | **オーディオ & マイク** | マイク入力 / スピーカー出力デバイスの独立選択 + マイクピークメーター |
| 6 | **アバター** | VRM ファイル変更 / 表情 / 初期視線 / 自由移動 / 吹き出し / アニメーション / 物理 / 照明 |
| 7 | **キャラクター** | キャラクタープリセット（アーキタイプ）/ 性格キーワード / 感情表現傾向 / 敬語 / 例示対話 / 自発的会話トグル |
| 8 | **音声（ローカル）** | STT エンジン（Whisper）· モデル選択（base/small/medium）+ ローカル TTS（Supertonic）音声・出力言語 + グローバル音声ショートカット |
| 9 | **画面観察** | 周期的な画面キャプチャ + Vision LLM 解析でアバターが能動発話 — キャプチャ対象 / 観察間隔 / 応答スタイル / 静かな時間 / 権限要求 |
| 10 | **モニター** | マルチモニター環境でアバターを表示するモニター選択 |
| 11 | **よく使う機能** | よく使う設定トグルを登録 → 右下の ✨ アイコン、または音声で即時呼び出し |
| 12 | **アプリ更新** | 現在バージョン表示 / 更新確認 / ダウンロード進捗 / 再起動（24時間ごと自動再確認） |
| 13 | **データ管理** | ダウンロード済み AI モデルデータの管理（アプリ削除時に自動削除されません） |
| 14 | **オープンソース / モデルライセンス** | 使用ライブラリ · AI サービス · モデルのライセンスと公式リンク |

> 設定パネルは右下の歯車アイコン、または macOS メニューバー `AMA → Settings...`（`⌘,`）から開けます。
...
AMA アバターを Google Gemini CLI（ACP）と接続して、コーディングエージェントとの双方向会話が可能です。

#### 前提条件

- [Gemini CLI](https://github.com/google-gemini/gemini-cli) インストール済み（`npm install -g @google/gemini-cli`）
- Gemini 認証済み（`gemini auth login` — Google OAuth または API キー）
- Node.js 20+

#### インストール状態確認

```bash
gemini --version       # バージョン出力
gemini auth print      # 認証状態確認
```

#### 設定手順

1. AMA アプリを起動
2. `設定 > AI モデル > Gemini CLI` を選択
3. CLI のインストール状態と認証状態が自動的に確認されます
4. 接続後、追加設定が可能:
   - **モデル**: 接続後 `session/new` で受け取った利用可能モデル一覧から選択
   - **作業フォルダ**: Gemini CLI がファイルを読み書きするディレクトリ（未指定時は `~/Documents`）
   - **承認モード** — ツール実行時の権限処理ポリシー:
     - `default` — 危険な作業は自動拒否（安全）
     - `auto_edit` — ファイル編集は自動承認、コマンド実行は拒否
     - `yolo` — すべてのツール実行を自動承認（信頼環境のみ）
     - `plan` — 実行せず計画のみ表示
5. AMA で会話 → Gemini CLI がコーディング作業を実行して応答 → アバターが TTS で読み上げ

#### 注意事項

- Gemini CLI はバックグラウンドで `gemini --experimental-acp` を自動的に起動します。別途ターミナル作業は不要です。
- `yolo` モードはファイル編集・コマンド実行・ターミナル作業すべてを自動承認します。**信頼できる環境でのみ使用**してください。
- 作業フォルダ外のファイルアクセスはブロックされます（セキュリティ隔離）。
- 画面観察（Screen Watch）が有効な場合、キャプチャ画像が ACP `image` ContentBlock として渡されます。
- Provider を他のモデルに切り替えると、Gemini CLI 接続は自動的に終了します。
