...
export default function GazeFollowController() {
  ...
  // 사용자가 아바타를 드래그/회전 중이면 커서 추적을 일시 중단한다.
  // 그렇지 않으면 드래그 중인 포인터를 따라 head/neck이 함께 돌면서
  // 체감상 몸통이 흔들리는 것처럼 보이는 문제가 생긴다.
  const isDragging = useAvatarStore((state) => state.isDragging);
  const isRotating = useAvatarStore((state) => state.isRotating);
  ...
  useFrame((_, delta) => {
    ...
    // 드래그·회전 중에는 커서를 따라가지 않는다. 누적된 head/neck 오프셋은 0으로
    // 부드럽게 되돌려 드래그를 놓았을 때 목이 갑자기 뻣뻣해 보이지 않게 한다.
    if (isDragging || isRotating) {
      const relaxBlend = 1 - Math.exp(-Math.max(0.001, delta) * HEAD_BLEND_RATE);
      headYawRef.current += (0 - headYawRef.current) * relaxBlend;
      headPitchRef.current += (0 - headPitchRef.current) * relaxBlend;
      // lookAt target도 해제 — eye 회전까지 중립으로 돌린다.
      vrm.lookAt.target = null;
      // 커서 "정지" 기준시각을 최신화해 드래그 중 saccade가 발동하지 않도록.
      lastCursorMoveRef.current = performance.now();
      return;
    }
