...
└──────────────────────────────────────────────────────────────┘
                   ...
                   ▼
┌──────────────────────────────────────────────────────────────┐
│ Cloud Services (Supertone API 프록시)                         │
│ - Supabase Edge Functions                                    │
│   ├─ supertone-tts   (JWT 검증 + 할당량 관리 + TTS 호출)    │
│   ├─ supertone-voices (음성 목록 프록시)                     │
│   └─ supertone-usage  (사용량 조회)                          │
│ - Supertone API (https://supertoneapi.com)                   │
└──────────────────────────────────────────────────────────────┘
```
...
6. 응답을 말풍선에 즉시 표시
7. `ttsRouter.playAudio()`로 TTS 재생 (엔진 자동 선택: supertonic 로컬 / supertone_api 클라우드)
8. 클라우드 TTS 실패 또는 할당량 소진 시 로컬 Supertonic으로 자동 폴백
9. 재생 중 립싱크/감정/제스처 동기화
...
### 4) Supertone API 클라우드 TTS 흐름
```
프런트엔드 (supertoneApiClient)
  → callEdgeFunction('supertone-tts', { text, voiceId, ... })
  → edgeFunctionClient: 세션 복원 → supabase.functions.invoke()
  → Supabase Edge Function (supertone-tts):
      JWT 검증 → 프리미엄 확인 → 월간 할당량 검사
      → Supertone API 호출 (x-sup-api-key)
      → tts_usage 기록 → 오디오 바이너리 반환
  → 프런트엔드: WAV 청크 결합 → HTMLAudio/WebAudio 재생
  → 할당량 소진 시: QuotaExceededError → 로컬 Supertonic 자동 폴백
```
...
- `premiumStore`:
  - 프리미엄 상태, Supertone 음성 목록, 할당량/사용량
