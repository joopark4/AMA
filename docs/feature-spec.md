...
### 정책
- **화면 경계 제약:** 화면 밖으로 이동 불가. 경계 도달 시 방향 전환
- **대각선 이동 지원**
...
### 정책
- **판별 방식:** 마우스 위치를 매 프레임 체크해 인터랙티브 영역 여부를 Tauri `set_ignore_cursor_events`로 전달
- **스케일 반영:** 아바타 인터랙티브 영역은 `avatar.scale`에 맞게 자동 계산
...
| 아바타 | VRM 파일, 이름, 성격 프롬프트, 스케일, 물리, 애니메이션, 조명 |
...
- **설정 버전 관리:** 스토어 버전 10. 구버전 설정값 자동 정규화
