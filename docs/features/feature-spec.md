...
### 기능 설명
AI 응답 텍스트를 음성으로 변환해 재생한다. 로컬 Supertonic(ONNX) 또는 클라우드 Supertone API를 선택할 수 있다.

### TTS 엔진

| 엔진 | 유형 | 요구 조건 |
|------|------|----------|
| `supertonic` | 로컬 (ONNX) | 기본값. 추가 설정 불필요 |
| `supertone_api` | 클라우드 | 프리미엄 구독 + 로그인 필요 |
...
### 지원 음성 (클라우드 Supertone API)
- Supertone API 서버에서 동적으로 조회
- 성별/언어/스타일 필터 지원
- 샘플 미리듣기 지원 (크레딧 소비 없음)
...
```
AI 응답 텍스트
  → supertoneApiClient.synthesize(text, options)
  → 300자 이하 청크 분할
  → Edge Function 프록시 호출 (JWT 인증 + 할당량 검사)
  → Supertone API TTS 생성
  → WAV 청크 결합 (100ms 무음 삽입)
  → HTML Audio API 재생 (실패 시 Web Audio API 폴백)
```

### 정책
- **엔진 선택:** `supertonic`(로컬) 또는 `supertone_api`(클라우드). 설정 패널에서 변경
- **로컬 처리 (supertonic):** 텍스트를 외부 서버로 전송하지 않음
- **클라우드 처리 (supertone_api):** Edge Function 프록시를 통해 Supertone API 호출. API 키는 서버 측에서만 관리
- **자동 폴백:** 클라우드 TTS 실패 또는 할당량 소진 시 로컬 Supertonic으로 자동 전환
...
| 음성 | STT 모델, TTS 음성, **대화·음성 언어**(로컬/프리미엄 공용), 글로벌 단축키 |
| 프리미엄 | TTS 엔진 선택, Supertone API 음성/모델/스타일, 사용량/할당량 |
...
- **설정 버전 관리:** 스토어 버전 13 (`settingsStore`). 구버전 설정값 자동 정규화
...
## 29. 프리미엄 음성 (Supertone API)

### 기능 설명
프리미엄 구독 사용자에게 Supertone API 기반 고품질 클라우드 TTS를 제공한다.

### 관련 파일
- `src/services/voice/supertoneApiClient.ts`
- `src/services/auth/edgeFunctionClient.ts`
- `src/stores/premiumStore.ts`
- `src/components/settings/PremiumVoiceSettings.tsx`
- `src-tauri/src/commands/http.rs`
- `supabase/functions/supertone-tts/index.ts`
- `supabase/functions/supertone-voices/index.ts`
- `supabase/functions/supertone-usage/index.ts`
...
```
앱 (supertoneApiClient)
  → edgeFunctionClient (세션 복원 + supabase.functions.invoke)
  → Supabase Edge Function
      ├─ JWT 검증
      ├─ 프리미엄 상태 확인
      ├─ 월간 할당량 검사
      ├─ Supertone API 호출 (서버 측 API 키)
      ├─ tts_usage 기록
      └─ 오디오 바이너리 + 할당량 헤더 반환
  → 프런트엔드: WAV 재생 + 할당량 UI 업데이트
```
...
프리미엄 확인
  ...
  → Supabase profiles 조회 (is_premium, plan_id)
  → 프리미엄 시 음성 목록/사용량 자동 로드

TTS 합성
  → ttsRouter.getActiveClient() — 엔진 설정에 따라 클라이언트 선택
  → supertone_api 선택 시: 할당량 소진 여부 확인 → 소진이면 로컬 폴백
  → 텍스트 300자 청크 분할 → Edge Function 호출
  → 응답 헤더에서 할당량 업데이트
  → QuotaExceededError 시 로컬 폴백 + 토스트 알림
```

### 음성 미리듣기
- 음성 목록에서 샘플 URL로 미리듣기 가능
- Tauri WebView에서 외부 URL 직접 fetch 불가 → Rust `fetch_url_bytes` 커맨드로 우회
- 크레딧 소비 없음

### 설정 UI (PremiumVoiceSettings)
- TTS 엔진 선택 (로컬 / 클라우드)
- 모델 선택 (`sona_speech_1`, `sona_speech_2`, `sona_speech_2_flash`)
- **대화·음성 언어** (모델별 지원 언어 동적 표시, 공용 `settings.tts.language`와 동기화)
  - Bella 등 voiceId가 비어있으면 프리미엄 엔진 전환 시 자동 주입
- 음성 선택 (성별/언어 필터 + 미리듣기)
- 스타일 선택 (선택된 음성의 지원 스타일)
- 감정 자동 매핑 토글 (AI 응답 감정 → TTS 스타일)
- 음성 미세 조정 (pitchShift, pitchVariance, speed)
- 사용량 카드 (할당량 바, 이번 달 요약, 최근 7일 차트)

### 정책
- **프리미엄 전용:** 로그인 + 프리미엄 구독 필요. 비프리미엄은 로컬 TTS만 사용
- **API 키 보호:** Supertone API 키는 Edge Function 서버 측에서만 관리. 클라이언트 미노출
- **할당량 관리:** 월간 크레딧 한도. 1초 = 1크레딧. 소진 시 로컬 자동 폴백
- **자동 폴백:** 클라우드 TTS 에러/할당량 소진 시 로컬 Supertonic 자동 전환 + 토스트 알림
- **Rate Limiting:** 클라이언트 측 1초 최소 간격 제한
- **텍스트 제한:** 청크당 300자 한도. 긴 텍스트는 자동 분할
- **감정 매핑:** `autoEmotionStyle` 활성화 시 AI 응답 감정을 Supertone 스타일로 자동 변환
...
| TTS 사용량 | Supabase | ✅ Supabase만 |
| 구독 정보 | Supabase | ✅ Supabase만 |
| 인증 토큰 | 로컬 only | ❌ 없음 |
