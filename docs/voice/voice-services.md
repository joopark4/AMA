...
- `VoiceSettings`:
  ...
  - **로컬(Supertonic) 출력 언어** (Pill 리스트, `settings.tts.language`): `auto / ko / en / ja / es / pt / fr`
    ...
    - **`settings.tts.language`는 supertonic 전용**. 프리미엄(supertone_api) 사용자가 zh 같은 supertonic-비지원 언어를 골라도 이 필드로 새지 않는다.
  - TTS 테스트 버튼: 현재 엔진 + 대화·음성 언어 조합에 맞춘 네이티브 샘플 문장 재생
- `PremiumVoiceSettings`:
  - TTS 엔진 선택 (로컬 / 클라우드)
  - 모델/언어/음성/스타일 선택
  - **프리미엄 출력 언어는 `settings.tts.supertoneApi.language`로 독립 관리**. 로컬 supertonic의 `tts.language`와 분리되어 zh/de 등 supertone-only 언어를 선택해도 로컬 설정에 영향 없음. 모델 변경 시 현재 언어가 새 모델에서 미지원이면 `en`으로 자동 정규화. 언어 변경 시 현재 voiceId가 새 언어 미지원이면 Bella → 첫 호환 음성 → 첫 음성 순으로 자동 전환.
...
- **엔진별로 별도 언어 필드를 단일 진실로 사용** (두 엔진 언어는 **독립 관리**):
  - `supertonic` → `settings.tts.language` (`auto / ko / en / ja / es / pt / fr`)
  - `supertone_api` → `settings.tts.supertoneApi.language` (모델 지원 언어 전체, 예: zh, de 등)
  - 프리미엄 측 언어 변경이 로컬 측 `tts.language`로 새지 않고, 그 반대도 마찬가지.
- **`auto`는 supertonic 측 한정으로 "앱 UI 언어(`settings.language`)를 그대로 따라간다"는 의미**. 텍스트 기반 감지는 하지 않는다.
- **엔진 제약**:
  - Supertonic(로컬)은 `ko/en/es/pt/fr`만 지원. `ja` 선택 또는 `auto`+UI=`ja`는 런타임 `en` 폴백 + UI 경고.
  - 프리미엄에서 LLM 프롬프트 템플릿이 없는 언어(zh 등)를 고르면 LLM 응답은 `en`, TTS 합성은 그대로 해당 언어로 진행. (TTS 발음만 zh, LLM 텍스트는 영어로 생성)

### 결정 테이블

| 엔진 | LLM 응답 언어 | TTS 합성 언어 |
|------|----------------|----------------|
| `supertonic` + `tts.language !== 'auto'` + 엔진 지원 | `tts.language` | `tts.language` |
| `supertonic` + `tts.language !== 'auto'` + Supertonic 미지원(`ja`) | `en` | `en` |
| `supertonic` + `tts.language === 'auto'` + UI 언어 지원 | `settings.language` | `settings.language` |
| `supertonic` + `tts.language === 'auto'` + UI=`ja` | `en` | `en` |
| `supertone_api` + `supertoneApi.language` 핵심 6개(ko/en/ja/es/pt/fr) | `supertoneApi.language` (native Layer 0 지시) | `supertoneApi.language` |
| `supertone_api` + `supertoneApi.language` 핵심 외(it/zh/de 등) | `supertoneApi.language` (영문 generic Layer 0 지시) | `supertoneApi.language` |
